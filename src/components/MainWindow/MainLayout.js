...
const MainLayout = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const transformedQueues = Object.entries(response.data.queues).map(
        ([queueNum, subQueues]) => ({
          queue: queueNum,
  ...
  const isTimeActive = (interval) => {
    ...
    const now = new Date();
    if (now >= end && now <= new Date(end.getTime() + 30 * 60 * 1000)) {
      return "wait";
    }
  ...
  const getTimeRemaining = (interval) => {
    ...
    if (now >= end && now <= new Date(end.getTime() + 30 * 60 * 1000)) {
      const waitTime = new Date(end.getTime() + 30 * 60 * 1000) - now;
  ...
  const getSubQueueStatus = (times) => {
    ...
    const hasWaitingPeriod = times.some(
      (interval) => isTimeActive(interval) === "wait"
    );
    if (hasWaitingPeriod) {
    ...
    <div className="grid">
      <div className="ag-format-container">
        <div className="item_title">
          Черги погодинних відключень у Рівненській області
        </div>
                ...
                <div className="ag-courses-item_link">
                  ...
                  <div className="ag-courses_item_subqueue">
                    {queue.subQueues.map((subQueue, index) => {
                      const subQueueStatusClass = getSubQueueStatus(
                        subQueue.times
                      );
                      const activeInterval = subQueue?.times.find(isTimeActive);
                      const nextInterval = getNextInterval(subQueue.times);
                      return (
                        <Link
                          to={`/queues/cherga/${queue.queue}/${subQueue.name}`}
                          key={subQueue.name}
                          className="ag-courses-item_subqueue_link"
                        >
